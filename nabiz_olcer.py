...
print("Sensör hazır. (Numpy'sız sürüm)")
print("Lütfen parmağınızı sensörün üzerine sabitçe yerleştirin...")

def mean(data):
    return sum(data) / len(data)

def rms(data, data_mean):
    # Karelerin ortalamasının karekökü (RMS hesaplama)
...
def calculate_bpm_spo2(red_data, ir_data):
    ...
    # Sinyaldeki değişimleri (AC bileşenlerini) bulma
    ir_ac = [x - ir_mean for x in ir_data]
    
    # Sıfır geçiş tespiti ile Nabız (BPM) tahmini
    zero_crossings = 0
    ...
    # Örnekleme hızı üzerinden kabaca BPM hesaplama
    # 100 örnek yakalamak ~1 saniye sürer (100Hz)
    total_seconds = len(ir_data) * 0.01 
    hz = (zero_crossings / 2) / total_seconds
    bpm = hz * 60
    
    # SpO2 için R Oranı hesabı
